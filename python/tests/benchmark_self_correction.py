...
async def run_scenario(scenario: Dict, feedback_mode: str):
    ...
    if noise_id is None:
        logger.error(f"Failed to find noise_id for {scenario['id']}. True search: {true_search}, Noise search: {noise_search}")
        return {
    ...
    for i, res in enumerate(final_results):
        logger.info(f"  {i+1}. ID={res['id']} Score={res.get('score', 'N/A')} Source={res['metadata'].get('source')} Text={res['text'][:30]}...")
...
async def run_benchmark():
    ...
    for scenario in config["scenarios"]:
        for mode in modes:
            ...
            print(f"{res['scenario_id']:<25} | {res['mode']:<10} | {res['p_at_1']:<5} | {res['noise_rank']:<10}")
    ...
    for mode, metrics in summary.items():
        print(f"Mode: {mode:<10} | Avg P@1: {metrics['avg_p1']:.2f} | Avg MRR: {metrics['avg_mrr']:.2f}")
