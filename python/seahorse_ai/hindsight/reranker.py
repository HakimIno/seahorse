    ...
    async def rerank(
        ...
        for i, doc in enumerate(documents):
            ...
            context_boost = self._calculate_context_boost(
                doc.get("text", ""), agent_role, current_task
            )

            # 3. Final Utility Formula integration
            base_score = (
                (rel * 0.4) + (access_score * 0.1) + (confidence * 0.1) + (context_boost * 0.2)
            ) * dist_decay
        ...
        reranked = sorted(documents, key=lambda x: x.get("utility_score", 0), reverse=True)

        return reranked[:top_n]

    async def _get_llm_relevance(
...
Return JSON: {{"0": score, "1": score, ...}}"""
        ...
        try:
            resp = await self.client.complete(
                [Message(role="user", content=prompt)], tier=self.tier
            )
            import json
