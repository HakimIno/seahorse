...
class HindsightRecaller:
    ...
    async def recall(
        ...
        merged = self._rrf_fuse(
            results_lists, k=k * 2
        )  # Get more candidates for textual deduplication
        ...
        merged = await self.reranker.rerank(
            query=query, documents=merged, agent_role=agent_role, current_task=current_task, top_n=k
        )
...
Answer:"""
    ...
    async def _vector_search(
        ...
        try:
            results = await self.pipeline.search(
                query, k=k, filter_metadata={"agent_id": agent_id} if agent_id else None
            )
            return results or []
        except Exception as e:
    ...
    async def _graph_search(
        self, query: str, agent_id: str | None, k: int, hops: int = 2
    ) -> list[dict[str, Any]]:
                ...
                for hit in path_hits:
                    ...
                    if rid not in seen_ids:
                        ...
                        graph_results.append(
                            {"id": rid, "graph_hit": True, "distance": hit["distance"]}
                        )
                        seen_ids.add(rid)
    ...
    async def _extract_entities_from_query(self, query: str) -> list[str]:
        ...
        prompt = (
            f"Extract proper nouns from this query as a comma-separated list: {query}\nEntities:"
        )
        try:
    ...
    def _rrf_fuse(
        self, results_lists: list[list[dict[str, Any]]], k: int, constant: int = 60
    ) -> list[dict[str, Any]]:
        ...
        for rank_list in results_lists:
            for i, doc in enumerate(rank_list):
                ...
                if doc_id not in doc_map or (
                    len(str(doc.get("text", ""))) > len(str(doc_map[doc_id].get("text", "")))
                ):
                    # Keep track of flags from the old version if it existed
                    prev_graph_hit = (
                        doc_map[doc_id].get("graph_hit", False) if doc_id in doc_map else False
                    )
                    prev_v_score = (
                        doc_map[doc_id].get("vector_score") if doc_id in doc_map else None
                    )
                ...
                if val is not None:
                    try:
                        ...
                        if (
                            target.get("vector_score") is None
                            or similarity > target["vector_score"]
                        ):
