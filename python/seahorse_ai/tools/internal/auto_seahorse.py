"""seahorse_ai.tools.internal.auto_seahorse — Complex multi-agent orchestration tools."""
...
async def execute_auto_seahorse(
    objective: str, context: str = "", team_hint: str | None = None
) -> dict[str, Any]:
    ...
    from seahorse_ai.engines.swarm import SeahorseCrew
    ...
    if not team_name or team_name == "GENERAL":
        ...
        team_name = (
            str(team_response.get("content", "RESEARCH")).strip().strip('"').strip("'").upper()
        )
    ...
    lang_instruction = (
        "IMPORTANT: You MUST speak in THAI." if is_thai else "REPLY in the user's language."
    )

    for agent in agents:
        agent.planner._identity_prompt = (
            agent.planner._identity_prompt or ""
        ) + f"\n\n{lang_instruction}"
