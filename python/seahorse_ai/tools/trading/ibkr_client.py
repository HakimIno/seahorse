...
try:
    ...
    HAS_IB_INSYNC = False
    logger.warning("ib_insync is not installed. IBKR features will be mock/disabled. Run: pip install ib_insync")
...
class IBKRClientManager:
    ...
    async def get_connection(self):
        ...
        if not HAS_IB_INSYNC:
            raise ImportError("ib_insync is required for Interactive Brokers integration. Please run `pip install ib_insync`.")
            
        if self._ib is None:
            self._ib = IB()
            
        if not self._ib.isConnected():
            logger.info("IBKR: Attempting to connect to TWS/Gateway on 127.0.0.1:4002 (Paper Trading)...")
            try:
                ...
                raise ConnectionError(f"Could not connect to IBKR TWS/Gateway on port 4002/7497. Is it running? Error: {e}")
