...
async def get_futures_live_price(symbol: str, expiry: str = "", exchange: str = "CME") -> dict[str, Any]:
...
async def get_futures_market_depth(symbol: str, exchange: str = "CME", rows: int = 5) -> dict[str, Any]:
...
@tool("Get the current live bid/ask/last price for a Stock (e.g., 'AAPL', 'TSLA') from Interactive Brokers.")
async def get_stock_live_price(symbol: str, exchange: str = "SMART", currency: str = "USD") -> dict[str, Any]:
...
async def get_forex_live_price(symbol: str, currency: str = "USD") -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "last": ticker.last or (ticker.bid + ticker.ask)/2 if (ticker.bid and ticker.ask) else None,
            "bid": ticker.bid,
            "ask": ticker.ask
        }
    except Exception as e:
