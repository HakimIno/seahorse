...
from seahorse_ai.tools.trading.ibkr_client import ibkr_manager, HAS_IB_INSYNC

logger = logging.getLogger(__name__)

@tool("Fetch the real-time Net Liquidation Value, Buying Power, and Margin from the connected Interactive Brokers account.")
async def get_ibkr_account_summary() -> dict[str, Any]:
    ...
    try:
        ...
        result = {}
        for item in summary:
            if item.tag in ("NetLiquidation", "AvailableFunds", "MaintMarginReq", "GrossPositionValue"):
...
async def place_ibkr_order(
    ...
    try:
        from ib_insync import MarketOrder, Stock, Forex, Future
        ib = await ibkr_manager.get_connection()
