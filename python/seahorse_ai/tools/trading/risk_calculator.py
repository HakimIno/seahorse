...
@tool("Calculate the exact Lot Size to trade to ensure the risk does not exceed the allowed percentage.")
async def calculate_position_size(
    ...
    try:
        if account_balance <= 0 or stop_loss_pips <= 0 or pip_value_per_lot <= 0:
            return {"error": "Account balance, stop loss pips, and pip value must be greater than 0."}
...
async def evaluate_kelly_criterion(win_rate: float, risk_reward_ratio: float) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "half_kelly_safe_risk": f"{round(fractional_kelly * 100, 2)}%" if kelly_percentage > 0 else "0%",
            "advice": summary
        }
    except Exception as e:
        return {"error": str(e)}
...
async def calculate_risk_of_ruin(win_rate: float, risk_reward_ratio: float, risk_per_trade_percent: float) -> dict[str, Any]:
    ...
    try:
        ...
        if risk_fraction >= 1.0:
            return {"probability_of_50_percent_loss": "100%", "advice": "Guaranteed ruin. Risk is too high."}
