...
class EntityHandler(BaseFastHandler):
    ...
    async def _handle_direct_extraction(self, prompt: str, history: list[Message] | None, start_t: float) -> AgentResponse | None:
        try:
            ...
            summary = "\n\n".join(kb_results)
            synthesis_prompt = f"Summarize these findings based on: {prompt}\n\nFindings:\n{summary}"
            ...
            return AgentResponse(content=content, steps=len(entities)+1, elapsed_ms=int((time.perf_counter() - start_t) * 1000))
    ...
    async def _handle_internal_analysis(self, prompt: str, start_t: float) -> AgentResponse | None:
        try:
            ...
            res = await self._llm.complete([Message(role="user", content=analysis_prompt)], tier="worker")
            content = str(res.get("content", res) if isinstance(res, dict) else res)
            return AgentResponse(content=content, steps=3, elapsed_ms=int((time.perf_counter() - start_t) * 1000))
        except Exception as e:
