...
class StoryHandler(BaseFastHandler):
    ...
    async def handle(
        ...
        try:
            # 1. Story Boarding
            boarding_msgs = [
                Message(role="user", content=STORY_BOARDING_PROMPT.format(prompt=prompt))
            ]
            boarding_res = await self._llm.complete(boarding_msgs, tier="fast")
            content_str = str(
                boarding_res.get("content", boarding_res)
                if isinstance(boarding_res, dict)
                else boarding_res
            )
            ...
            synthesis_input = "\n\n".join(story_results)
            synth_msgs = [
                Message(role="user", content=STORY_SYNTHESIS_PROMPT.format(results=synthesis_input))
            ]
