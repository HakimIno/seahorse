...
class PolarsHandler(BaseFastHandler):
    ...
    async def handle(self, prompt: str, history: list[Message] | None, start_t: float, **kwargs: Any) -> AgentResponse | None:
        try:
            ...
            files = [f for f in os.listdir(".") if f.lower().endswith((".parquet", ".csv", ".json"))]
            if os.path.exists("workspace"):
                files.extend([f"workspace/{f}" for f in os.listdir("workspace") if f.lower().endswith((".parquet", ".csv", ".json"))])
            ...
            # 2. Querying
            data_res = await self._tools.call("duckdb_query_json", {"sql_query": sql, "max_rows": 10000})
    ...
    async def _handle_scatter(self, data: dict, rows: list, x_col: str | None, y_col: str | None) -> tuple[str, str]:
        # Implementation of previous scatter plot logic
        cols = list(rows[0].keys()) if rows else []
        if not x_col and len(cols) >= 1: x_col = cols[0]
        if not y_col and len(cols) >= 2: y_col = cols[1]
        ...
        option = {
            ...
            "series": [{"data": data_points[:2000], "type": "scatter", "symbolSize": 6, "itemStyle": {"color": "#00d9ff"}}]
        }
